...
def sprint():
    ...
    drive.toPos(vec2(30*8,15), background=True)
    red = False
    while not drive.robot.pos.x >= 30*8:
        drive.runTasks()
    drive.stopTasks()
...
def medved():
    ...
    up(True)
    drive.toPos(vec2(75,15), backwards=True)
    drive.toPos(vec2(45,15), backwards=True)
    #drive.rotate(90)
...
def mechaline(time, speed):
    timer = StopWatch()
    while timer.time() > time:
        drive.robot.update()
        drive.robot.lM.m_motor.run(speed)
        drive.robot.rM.m_motor.run(speed)
    drive.robot.lM.m_motor.stop()
    drive.robot.rM.m_motor.stop()
